...
public class EnemyTEST : MonoBehaviour
{
    ...
    public void takeDamage(float d)
    {
        ...
        if (state == EnemyStates.scan) { alert(true, true); }
    ...
    void alert(bool b, bool others)
    {
        if (b && state == EnemyStates.scan)
        {
            createAlertPopup();
            ...
            Collider[] colliders = Physics.OverlapSphere(transform.position, lockonDistance);
            foreach (Collider collider in colliders)
            {
                if (collider.gameObject.GetComponent<EnemyTEST>() != null)
                {
                    collider.gameObject.GetComponent<EnemyTEST>().alert(b, false);
                }
            }
        }
    }
    ...
    void Update()
    {
        ...
        switch (state)
        {
            case EnemyStates.scan:
                alert(scanForPlayer(scanDistance), true);
                break;
